...
def manual_oversample(X, y):
    """Replace SMOTE with simple random oversampling — no imblearn needed."""
    np.random.seed(42)
    idx_min = np.where(y == 1)[0]
    idx_maj = np.where(y == 0)[0]
    idx_up  = np.random.choice(idx_min, size=len(idx_maj), replace=True)
    idx_all = np.concatenate([idx_maj, idx_up])
    np.random.shuffle(idx_all)
    return X[idx_all], np.array(y)[idx_all]
...
<div class="imbalance-banner">
  ...
  <p style="font-size:13px;color:#94a3b8;line-height:1.6;margin-bottom:10px;">
    ...
    Enable <strong>Oversampling</strong> below to balance the minority class for fairer predictions.
  </p>
...
use_smote = st.toggle("⚖️ Apply Oversampling (balance classes before training)", value=True)
...
def compute_metrics(use_smote_flag):
    ...
    if use_smote_flag:
        X_tr_sc, y_tr = manual_oversample(X_tr_sc, np.array(y_tr))
    out = {}
...
st.markdown("""
<div style="border-top:1px solid #1e2d45;padding-top:20px;margin-top:10px;">
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
    ...
    <span style="font-family:DM Mono,monospace;font-size:11px;color:#64748b;">
      Oversampling · StandardScaler · LabelEncoder · OneHotEncoder
    </span>
  </div>
  <div style="margin-top:14px;padding-top:14px;border-top:1px solid #1e2d45;
       display:flex;align-items:center;justify-content:center;gap:16px;flex-wrap:wrap;">
    <span style="font-family:Syne,sans-serif;font-size:13px;color:#94a3b8;">
      Built by <strong style="color:#00e5ff;">Mehran Mushtaq</strong>
    </span>
    <span style="color:#1e2d45;">|</span>
    <a href="https://github.com/mehranmushtaq" target="_blank"
       style="font-family:DM Mono,monospace;font-size:12px;color:#00e5ff;
